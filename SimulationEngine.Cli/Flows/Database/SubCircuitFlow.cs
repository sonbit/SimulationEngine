...
public sealed class SubCircuitFlow(IInputOutput inputOutput, IRenderer renderer, ISubCircuitService service)
{
    ...
    public async Task RunMenuAsync(SubCircuit subCircuit)
    {
        while (true)
        {
            ...
            var menuOption = await inputOutput.SelectEnumAsync<MenuOptions>($"[bold]Simulation: {subCircuit.Title} ({subCircuit.Id})[/]");

            switch (menuOption)
            {
                case MenuOptions.Simulate:
                case MenuOptions.SimulateNormalized:
                    await SimulationEntry.SimulateAsync(subCircuit, renderer, null, menuOption == MenuOptions.SimulateNormalized);
                    break;

                case MenuOptions.SimulateFile:
                case MenuOptions.SimulateFileNormalized:
                    var fileInfo = await inputOutput.PickFileAsync("Pick a test file", Environment.CurrentDirectory, "*.txt");
                    await SimulationEntry.SimulateAsync(subCircuit, renderer, fileInfo, menuOption == MenuOptions.SimulateFileNormalized);
                    break;
